...
export function targetValidationRows(horizon = 12) {
  ...
  // validateEnsemble keeps origins non-overlapping and requires >=18 resolved checks.
  // At longer horizons the 30% warm-up consumes more history, so target rows scale
  // conservatively at ~26 observations per forecast horizon plus a safety margin.
  return Math.max(480, Math.ceil(26 * h + 32));
}
