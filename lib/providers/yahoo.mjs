...
const RANGE_ORDER=['1d','5d','1mo','3mo','6mo','ytd','1y','2y','5y','10y','max'];
...
function cappedRange(requested,maxRange){
  const requestedIndex=RANGE_ORDER.indexOf(requested),maxIndex=RANGE_ORDER.indexOf(maxRange);
  if(requestedIndex<0||maxIndex<0)return maxRange;
  return requestedIndex>maxIndex?maxRange:requested;
}
export function normalizeYahooIntervalRange(interval='1d',range='2y'){
  const normalizedInterval=ALLOWED_INTERVALS.has(interval)?interval:'1d';
  const requestedRange=ALLOWED_RANGES.includes(range)?range:'2y';
  let normalizedRange=requestedRange;
  if(normalizedInterval==='1m') normalizedRange=cappedRange(requestedRange,'5d');
  else if(['2m','5m','15m','30m','90m'].includes(normalizedInterval)) normalizedRange=cappedRange(requestedRange,'1mo');
  else if(['60m','1h'].includes(normalizedInterval)) normalizedRange=cappedRange(requestedRange,'1y');
  return {interval:normalizedInterval,range:normalizedRange,requestedRange,rangeAdjusted:normalizedRange!==requestedRange};
}
...
export function deriveChartMeta(m={},candles=[],{interval='1d',range='2y',nowSec=Math.floor(Date.now()/1000)}={}){
  ...
  const regularPeriod=validPeriod(m?.currentTradingPeriod?.regular);
  ...
  return {
    ...
    regularSessionStart:regularPeriod?.start??null,
    regularSessionEnd:regularPeriod?.end??null,
...
export async function chart(symbol, interval='1d', range='2y'){
  ...
  const normalized=normalizeYahooIntervalRange(interval,range);
  interval=normalized.interval; range=normalized.range;
  ...
  const m=r.meta||{};
  const meta={...deriveChartMeta(m,candles,{interval,range}),requestedRange:normalized.requestedRange,rangeAdjusted:normalized.rangeAdjusted};
