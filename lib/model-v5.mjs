...
function normalizeWeights(raw) {
  ...
  // Keep a meaningful no-change anchor. This makes the ensemble prove that
  // taking market risk adds value rather than allowing the baseline to vanish.
  const floor = 0.10;
...
export function weightsFromCandidateMae(candidateMae = null) {
  ...
  for (const name of CANDIDATE_NAMES) {
    // Exponential loss weighting sharply penalizes persistently inferior models
    // without ever giving any single candidate absolute authority.
...
export function validateEnsemble(candles, horizon = 12, maxChecks = 80) {
  ...
  for (const i of origins.slice(-maxChecks)) {
    ...
    const pUp = clamp(legacy.direction?.up ?? 0.5, 1e-6, 1 - 1e-6);
...
function shiftedPoints(legacy, ensembleReturn, current, horizon, publishable) {
...
export function forecastV5(candles, horizon = 12, validation = null) {
  ...
  const center = publishable ? current * Math.exp(ensemble.return) : decisionState === 'RESEARCH_ONLY' ? current * Math.exp(ensemble.return) : null;
  return {
    ...legacy,
    current,
    center,
    ...
    points: shiftedPoints(legacy, ensemble.return, current, horizon, decisionState !== 'ABSTAIN'),
  };
}
