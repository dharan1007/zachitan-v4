import { cachedFetchText, provenance } from '../http.mjs';
...
function pairValue(row, base, quote) {
  ...
  // ECB reference table expresses one EUR in quote currency units.
...
export async function ecbPair(symbol, { days = 3650 } = {}) {
  ...
  const text = await cachedFetchText(ECB_HIST, { ttlMs: 6 * 60 * 60 * 1000, timeoutMs: 10000 });
  const rows = parseCsv(text);
  const cutoff = Date.now() - Math.max(180, days) * 86400_000;
  const series = rows.map(row => {
    const time = Math.floor(Date.parse(`${row.Date}T00:00:00Z`) / 1000);
    const close = pairValue(row, base, quote);
  ...
  if (series.length < 30) throw new Error(`ECB returned too little history for ${base}/${quote}`);
