...
import { modelHistoryPlan, sliceDisplayCandles } from '@/lib/model-history.mjs';
...
function analysisKey(provider, symbol, interval, horizon, modelCandles) {
  const first = modelCandles.at(0);
  const last = modelCandles.at(-1);
  return `${provider}:${symbol}:${interval}:${horizon}:${first?.time || 0}:${last?.time || 0}:${last?.close || 0}:${modelCandles.length}`;
}
...
function analyzeMarket(provider, symbol, interval, horizon, modelCandles) {
  const key = analysisKey(provider, symbol, interval, horizon, modelCandles);
  const cached = analysisCache.get(key);
  if (cached && Date.now() - cached.at < 15 * 60_000) return { ...cached.value, cacheHit: true };
  const validation = validateEnsemble(modelCandles, horizon, 36);
  const modelForecast = forecastV5(modelCandles, horizon, validation);
  const result = { validation, modelForecast, indicators: indicators(modelCandles), cacheHit: false, analysisIdentity: key };
...
async function market(u) {
  ...
  let historyPlan = modelHistoryPlan({ provider, interval, range, horizon, displayBars });
  let modelCandles, meta, quote = null, book = null, trades = null, sourceProvenance;

  if (provider === 'coinbase') {
    modelCandles = await cb.candles(symbol, g, historyPlan.modelBars);
    quote = await cb.ticker(symbol);
    meta = {
      name: symbol, assetClass: 'crypto', currency: quote.currency, exchange: quote.exchange,
      price: quote.price, previousClose: null, marketState: '24/7 live',
      timezone: 'UTC', dataGranularity: interval, interval,
      range: `${range} · display up to ${historyPlan.displayBars} bars · model up to ${historyPlan.modelBars} bars`,
    };
    ...
    const d = await amfi.history(symbol, historyPlan.modelDays);
    modelCandles = d.candles; meta = { ...d.meta, range, modelDays: historyPlan.modelDays }; sourceProvenance = d.provenance;
    ...
    const d = await ecb.fx(base, quoteCurrency, historyPlan.modelDays);
    modelCandles = d.candles; meta = { ...d.meta, range, modelDays: historyPlan.modelDays }; sourceProvenance = d.provenance;
  } else if (provider === 'yahoo') {
    const effective = yf.normalizeYahooIntervalRange(interval, historyPlan.modelRange);
    historyPlan = { ...historyPlan, modelRange: effective.range, historyLimited: historyPlan.modelRange !== effective.range };
    const d = await yf.chart(symbol, effective.interval, effective.range);
    modelCandles = d.candles;
    meta = { ...d.meta, interval: effective.interval, range, modelRange: effective.range };
    sourceProvenance = yf.provenance;
    quote = {
  ...
  const displayCandles = sliceDisplayCandles(modelCandles, historyPlan);

  const session = marketSessionPolicy({ provider, meta, interval: meta.interval || interval });
  const analysis = analyzeMarket(provider, symbol, meta.interval || interval, horizon, modelCandles);
  const timedForecast = addForecastObservationTimes(analysis.modelForecast, modelCandles, meta, provider, meta.interval || interval);
  ...
  return {
    ...
    quality: marketQuality(displayCandles, meta, provider, session, normalized.diagnostics),
    provenance: sourceProvenance,
    compute: {
      validationOriginsMax: 36,
      warmAnalysisCacheHit: analysis.cacheHit,
      modelRows: modelCandles.length,
      displayRows: displayCandles.length,
      requestedRange: range,
      modelRange: historyPlan.modelRange || null,
      historyLimited: !!historyPlan.historyLimited,
      targetValidationRows: historyPlan.targetRows || null,
    },
