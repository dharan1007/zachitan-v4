...
export function regimeDiagnostics(candles) {
  if (!Array.isArray(candles) || candles.length < 40) {
    return { available: false, severe: false, elevated: false, reason: 'Need more history for regime diagnostics.' };
  }
  const last = candles.length - 1;
  const close = candles.map(x => +x.close);
  const returns = [];
  for (let i = Math.max(1, last - 120); i < last; i++) {
    if (close[i - 1] > 0 && close[i] > 0) returns.push(Math.log(close[i] / close[i - 1]));
  }
  const currentReturn = close[last - 1] > 0 && close[last] > 0 ? Math.log(close[last] / close[last - 1]) : NaN;
  const med = median(returns);
  const robustSigma = 1.4826 * mad(returns) || stdev(returns) || 0;
  const jumpZ = robustSigma > EPS && Number.isFinite(currentReturn) ? Math.abs(currentReturn - med) / robustSigma : 0;
  const atr = atrAt(candles, last, 14);
  const atrPct = close[last] > 0 ? atr / close[last] : NaN;
  const absMove = Number.isFinite(currentReturn) ? Math.abs(Math.expm1(currentReturn)) : NaN;
  const volume = candles.map(x => Number.isFinite(+x.volume) ? +x.volume : 0);
  const volumeZ = robustZ(volume[last], volume.slice(Math.max(0, last - 60), last));
  const absoluteGuard = Number.isFinite(absMove) && absMove >= 0.08;
  const statisticalGuard = jumpZ >= 8 && absMove >= 0.02;
  const atrGuard = Number.isFinite(atrPct) && atrPct > 0 && absMove >= Math.max(0.03, atrPct * 5.5);
  const severe = absoluteGuard || statisticalGuard || atrGuard;
  const elevated = severe || jumpZ >= 4.5 || (Number.isFinite(absMove) && absMove >= 0.04);
  return {
    available: true,
    severe,
    elevated,
    currentReturn,
    absoluteMove: absMove,
    jumpZ,
    robustSigma,
    atrPct,
    volumeZ,
    reason: severe
      ? 'Latest observation is an out-of-distribution jump/regime break relative to recent returns and range.'
      : elevated
        ? 'Latest observation is unusually large relative to the recent regime.'
        : null,
  };
}
...
export function forecast(candles, horizon = 12, validation = null) {
  ...
  if (!samples.length) {
    return { available: false, reason: 'Not enough clean history for this timeframe.', effectiveN: 0, regime };
  }
  ...
  const rawCenter = current * Math.exp(rawMedian * shrink);
  const ranges = {};
  ...
  const rawPoints = checkpointBars(horizon).map(bar => {
  ...
  const positiveSkill = validation?.available && validation.skillVsNoChange > 0 && validation.brierSkillVs50 > 0;
  ...
  const abstainReason = regime.severe
    ...
    : negativeMeasuredSkill
      ? 'Walk-forward validation did not beat both unchanged-price and 50/50 probability baselines.'
      : null;
  const modelStatus = regime.severe
  ...
  const center = decisionState === 'ABSTAIN' ? null : rawCenter;
  const points = rawPoints.map(point => decisionState === 'ABSTAIN'
    ? { ...point, price: null, change: null }
    : point);
